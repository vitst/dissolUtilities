...
class SeededOutlet2blocks(gbm.AbstractBaseGenerator):
  '''
  This generator creates a fracture seeded at the outlet. Cosine like seed is at the inlet at Lx/2.
  blockMeshDict consist of two blocks. First block contains the seed and the second
  is a flat fracture.
  '''
  def __init__(self):
    self.__genName__ = 'outseeded'
  ...
  def createBlockMeshDict(self, dictFileName):
    ...
    fileCont += '    ({0:8g} {1:8g} {2:8g})\n'.format(-Lx/2., -Ly/2., Lz-seed_z)     # 4
    fileCont += '    ({0:8g} {1:8g} {2:8g})\n'.format( Lx/2., -Ly/2., Lz-seed_z)     # 5
    fileCont += '    ({0:8g} {1:8g} {2:8g})\n'.format( Lx/2.,  Ly/2., Lz-seed_z)     # 6
    fileCont += '    ({0:8g} {1:8g} {2:8g})\n'.format(-Lx/2.,  Ly/2., Lz-seed_z)     # 7
    ...
    fileCont += '    hex (0  1  2  3  4  5  6  7) ({0:d} {1:d} {2:d})\n' \
                ...
                '        {5:g}\n' \
                '      )\n'. \
                format(x_res, y_res, z_res, x_Gs, y_Gs, 1.0/z_G)
